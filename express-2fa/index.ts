...
app.post("/loginauth", async (request, response) => {
  ...
  try {
    rows = (await db.getUsername(username)) as { password: string }[];
  } catch (e) {
...
app.post("/auth", async (request, response) => {
  ...
  if (sameUsers.length > 0) {
    ...
    try {
      telnyx_response = await telnyx.verifications.smsVerify({
...
app.post("/verify", async (request, response) => {
  ...
  try {
    telnyx_response = await telnyx.verifications.verifyByPhoneNumber(
      request.cookies.number,
      {
        code: code_try,
        verify_profile_id: process.env.TELNYX_VERIFY_KEY!,
      }
